export default {
  ...
  render() {
    return (
      <div class={[`${prefix}-form-item`, { 'has-error': this.hasError }]} data-field-name={this.prop}>
        { !this.hideLabel && <div class={`${prefix}-form-item-label`}>{this.label}</div> }
        <div class={`${prefix}-form-item-field`}>
          { this.$slots.default }
          { this.hasError && <div class={`${prefix}-form-item-errormsg`}>{this.errObj[this.prop]}</div> }
        </div>
      </div>
    );
  },
