<template>
  <div :class="`${prefix}-form-area ${noBoard ? 'no-board' : ''}`">
    ...
    <div :class="`${prefix}-form-area-body`">
      <component :is="`${prefix}-form`" v-model="value" :rules="innerRules" v-on="$listeners" >
        <component
          ...
          :prop="item.prop"
          :label="item.label">
          <component v-if="item.type === 'input'" :is="`${prefix}-input`" v-model="value[item.prop]" />
          <component v-else-if="item.type === 'copyInput'" :is="`${prefix}-input`" readonly v-model="value[item.prop]" >
            <component :is="`${prefix}-button`" icon="copy" class="copy-btn" slot="append" @click.native="handleCopy"/>
          </component>
          <span v-else-if="item.type === 'text'">
            <template v-if="item.filter">{{useFilter(value[item.prop],item.filter)}}</template>
            <template v-else-if="item.template">{{renderTpl(value,item.template)}}</template>
            <slot v-else-if="item.slot" :name="item.prop" :data="value" >slot name: {{item.prop}}</slot>
          ...
          <span v-else><span style="color:red;">todo</span> type:{{item.type}} <br />value:{{value[item.prop]}}</span>
        </component>
      </component>
    </div>
  </div>
...
export default {
  ...
  methods: {
    useFilter(origin, filter) {
      const [filtername, ...args] = typeof filter === 'string' ? [filter] : filter;
