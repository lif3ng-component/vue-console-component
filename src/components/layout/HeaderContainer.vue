...
export default {
  ...
  mounted() {
    ...
    aTagVnodes.forEach(({ elm }) => elm.style.lineHeight = `${parentHeight}px`);
    const drowdownMenuVnodes = defaultSlots.filter(({ componentOptions }) => componentOptions && componentOptions.tag === `${prefix}-dropdown-menu`);
