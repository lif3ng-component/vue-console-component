...
import SidebarDemoArea from './SidebarDomeArea.vue';
...
export function renderLink(h, to, text, active) {
  return h(
    "router-link",
    {
      props: {
...
const renderSideDemoArea = (h,path) => {
  return h(SidebarDemoArea,{
      props: {
        path
      }
  })
  ...
  return h("ul", { class: "sidebar-sub-headers" }, [
    ...children.map(c => {
      ...
      return h("li", { class: "sidebar-sub-header" }, [
        ...
        renderChildren(h, c.children, path, route, maxDepth, depth + 1, true)
      ]);
    }),
    ...(!notRenderDemo ? [renderSideDemoArea(h,path)] : [])
  ]);
}
